...
int s21_sprintf(char *str, const char *format, ...) {
    ...
    while (format[j] != '\0') {
        if (format[j] == '%') {
            ...
            if (format[j] == 'l')
                specifier_ll(str, format, &i, &j, args);
            else if (format[j] == 'c')
                specifier_c(str, &i, &j, args);
            else if (format[j] == 'd')
                specifier_d(str, &i, &j, args);
            else if (format[j] == 'f')
                specifier_f(str, &i, &j, args);
            else if (format[j] == 's')
                specifier_s(str, &i, &j, args);
            else if (format[j] == 'u')
                specifier_u(str, &i, &j, args);
            else if (format[j] == '%')
                specifier_per(str, format, &i, &j);
        }
        str[i++] = format[j++];
    }
    str[i++] = '\0';
