...
int main() {
    char buffer[1028] = {0};
    s21_sprintf(buffer, "Hello %.*d", 12,  -9892344);
    printf("%s\n", buffer);
    sprintf(buffer, "Hello %.*u", 12, -9892344);
    printf("%s\n", buffer);
...
void accuracy(char *str, const char* format, int *i, int *j, va_list args) {
   ...
   int ac = va_arg(args, int);
    ...
    if (f == '.') {
        ...
        double h = round((f - (long long)f)*a);
        h = h / a;
        while (count < ac) {
            a = a/10;
            h = h * 10;
    ...
    if ((format[*j+1] >= '0' || format[*j+1] <= '9') && format[*j+1] != '*'){
        if (format[*j+2] == 'd') {
            long int d = va_arg(args, int);
        if (d < 0) {
        str[(*i)++] = '-';
...
void specifier_f(char *str, int *i, va_list args) {
    ...
    if (f == '.') {
        ...
        str[(*i)++] = '.';
        double h = (round)((f - (long long)f)*1000000)/1000000;
        while (count < 6) {
            h = h * 10;
            str[(*i)++] = (int)(long long)h % 10 + '0';
            h = (round)((f - (long long)f)*1000000)/1000000;
            count++;
        }
    }
}
